...
pub struct TunConfig {
    ...
    /// Whether to automatically configure routing for the interface
...
impl TunConfig {
    ...
    pub fn enable_routing(&mut self, value: bool) -> &mut Self {
        self.enable_routing = Some(value);
        self
    }
    ...
    pub fn create_as_async(&self) -> std::io::Result<AsyncDevice> {
        ...
        if let Some(address) = self.address
            && self.enable_routing.unwrap_or(true)
        {
            match address {
                IpAddr::V4(ipv4_addr) => {
                    let netmask = self
                        .prefix
                        .and_then(|x| Some(x.min(MAX_PREFIX_LEN_IPV4)))
                ...
                IpAddr::V6(ipv6_addr) => {
                    let netmask = self
                        .prefix
                        .and_then(|x| Some(x.min(MAX_PREFIX_LEN_IPV6)))
