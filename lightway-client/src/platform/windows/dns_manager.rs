...
impl DnsManager {
    ...
    fn add_nrpt_rule(dns_server: IpAddr) -> Result<(), DnsManagerError> {
        ...
        let cmd = format!(
            "Add-DnsClientNrptRule -Comment 'lightway-dns' -Namespace '.' -NameServers {server}"
        );
...
impl DnsSetup for DnsManager {
    fn set_dns(&mut self, dns_server: IpAddr) -> Result<(), DnsManagerError> {
        ...
        // always remove it first in case the rule is hanging after client crash
...
mod tests {
    ...
    fn get_configured_nrpt_server() -> Option<String> {
        ...
        let server = stdout.splitn(2, ':').nth(1).map(|s| s.trim().to_string());
