...
use lightway_app_utils::{
    ConnectionTicker, DplpmtudTimer, EventStream, EventStreamCallback, connection_ticker_cb,
};
use lightway_core::{
    BuilderPredicates, ClientContextBuilder, Connection, ConnectionError, ConnectionType, Event,
    EventCallback, IOCallbackResult, InsideIOSendCallbackArg, OutsidePacket, State,
    WolfsslConnection, ipv4_update_destination, ipv4_update_source,
};
...
use crate::keepalive::{self, Config as KeepaliveConfig, Keepalive};
#[cfg(desktop)]
use crate::route_manager::{RouteManager, RouteMode};
use crate::{ClientConfig, ClientConnectionConfig, ClientTransport};
#[cfg(feature = "debug")]
...
use super::{ClientIpConfigCb, ClientResult, ConnectionState, io};
...
/// WolfSSL/DTLS-specific transport configuration.
#[derive(educe::Educe)]
#[educe(Debug)]
pub struct WolfsslClientTransport {
        ...
        match &event {
            Event::StateChanged(state) => {
                if matches!(state, State::Online) {
                    ...
                    {
                        tracing::error!("Error encountered when trying to toggle encoding. {}", e);
                    }
...
pub async fn encoding_request_task<ExtAppState: Send + Sync>(
    ...
    while let Some(enable) = signal.recv().await {
        ...
        if let Err(e) = conn.lock().unwrap().set_encoding(enable) {
            tracing::error!(
                "Error encountered when trying to send encoding request. {}",
                e
            );
        }
    }
...
>(
    ...
    let ClientTransport::Wolfssl(wolfssl) = &config.transport;
    ...
    if let Some(size) = wolfssl.sndbuf {
        outside_io.set_send_buffer_size(size.as_u64().try_into()?)?;
    }

    if let Some(size) = wolfssl.rcvbuf {
    ...
    .when_some(wolfssl.pmtud_base_mtu, |b, mtu| b.with_pmtud_base_mtu(mtu))
    .when_some(server_config.server_dn, |b, sdn| {
        b.with_server_domain_name_validation(sdn)
    })
    .when(connection_type.is_datagram() && wolfssl.enable_pmtud, |b| {
        b.with_pmtud_timer(pmtud_timer)
    });

    #[cfg(feature = "postquantum")]
    let conn_builder = conn_builder.when(wolfssl.enable_pqc, |b| b.with_pq_crypto());

    #[cfg(feature = "debug")]
    let conn_builder = conn_builder.when_some(wolfssl.keylog.clone(), |b, k| {
    ...
    let keepalive_config = keepalive::Config {
        ...
        continuous: wolfssl.continuous_keepalive,
        tracer_trigger_timeout: Some(wolfssl.tracer_packet_timeout),
    };
...
>(
    ...
    let inside_io = match &config.inside_io {
        ...
        #[cfg(feature = "io-uring")]
        None if wolfssl.enable_tun_iouring => Arc::new(
            io::inside::Tun::new_with_iouring(
                &config.tun_config,
                config.tun_local_ip,
                config.tun_dns_ip,
                wolfssl.iouring_entry_count,
                wolfssl.iouring_sqpoll_idle_time,
            )
            .await?,
        ),
        None => Arc::new(
    ...
    let preferred_wait = wolfssl.preferred_connection_wait_interval;
