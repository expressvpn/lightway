...
async fn network_event_coordinator(
    ...
    // Failed re-pins retry while the network settles: 500ms after the first
    // failure, then 1s per retry. The ceiling covers PEAP phase 2 stalls
    // which can hold for up to 25s.
    const MAX_REPIN_RETRIES: u32 = 30;
    ...
    loop {
        ...
        // A fresh event restarts the retry budget but keeps any nudge owed
        ...
        match route_updater.check_and_update_server_route().await {
            Err(e) if state.attempt < MAX_REPIN_RETRIES => {
                if state.attempt < 7 {
                    tracing::debug!("Server route update failed ({e:?}), retrying");
                } else {
                    tracing::warn!("Server route update failed ({e:?}) more than 6s, retrying");
                }
                state.next_at = tokio::time::Instant::now()
                    + std::time::Duration::from_millis(if state.attempt == 0 {
                        500
                    } else {
                        1_000
                    });
