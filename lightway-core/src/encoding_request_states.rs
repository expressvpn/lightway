...
#[derive(Default)]
pub struct EncodingRequestStates {
    ...
    /// The encoding request packet that has yet to be acknowledged by the server
    /// Used by Client only.
...
impl EncodingRequestStates {
    fn retransmit_wait_time(&self) -> Duration {
        ...
        INITIAL_WAIT_TIME * (1 + self.retransmissions_counter)
    }
}

pub type ScheduleEncodingRequestRetransmitCb = fn(d: std::time::Duration);
