...
impl ConnectionManager {
    pub(crate) fn new(ctx: ServerContext<ConnectionState>) -> Arc<Self> {
    ...
    pub(crate) fn clean_up_stale_pkt_accumulator_states(&self) {
        let connections = self.connections.lock().unwrap().get_all_connections();

        for conn in connections {
            conn.clean_up_stale_pkt_accumulator_states();
        }
    }

    pub(crate) fn flush_pkt_accumulator(&self) -> Vec<(Arc<Connection>, ConnectionResult<()>)> {
        let connections = self.connections.lock().unwrap().get_all_connections();

        connections
            .into_iter()
            .map(|conn| (conn.clone(), conn.flush_pkt_accumulator()))
            .collect()
    }
