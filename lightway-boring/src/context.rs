...
impl ContextBuilder {
    ...
    ///
    /// For [`RootCertificate::PemFileOrDirectory`] a directory is loaded like
    /// wolfSSL's `wolfSSL_CTX_load_verify_locations`: every regular file in
    /// the directory (non-recursive) is parsed as PEM. Files containing no
    /// PEM certificates are skipped; a directory yielding no certificates at
    /// all is an error.
    pub fn with_root_certificate(mut self, cert: RootCertificate) -> Result<Self> {
        ...
        let certs = match cert {
            ...
            RootCertificate::PemFileOrDirectory(path) if path.is_dir() => {
                let mut certs = Vec::new();
                for entry in std::fs::read_dir(path).map_err(TlsError::Io)? {
                    let file = entry.map_err(TlsError::Io)?.path();
                    if !file.is_file() {
                        continue;
                    }
                    let data = std::fs::read(&file).map_err(TlsError::Io)?;
                    certs.extend(X509::stack_from_pem(&data).map_err(TlsError::BoringSSL)?);
                }
                if certs.is_empty() {
                    return Err(TlsError::InvalidParameter(format!(
                        "no PEM certificates found in directory: {}",
                        path.display()
                    ))
                    .into());
                }
