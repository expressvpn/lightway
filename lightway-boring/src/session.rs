    ...
    IOCB: IOCallbacks,
{
    ...
    /// Get TLS/DTLS protocol version
    fn tls_protocol_version(&self) -> ProtocolVersion {
        if self.is_dtls {
    ...
    IOCB: IOCallbacks + std::fmt::Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Session")
            .field("protocol_version", &self.tls_protocol_version().as_str())
